...
class ORFFinder():
    ...
    def get_all_orfs(self , readingframe = 1):
        '''
        ...
        orfs = []
        while start<=len(self.sequence)-3:
            end = self.get_next_orf(self.sequence, start)
            if end>start:
                #start, end, length
    ...
    def get_sequences(self, orfs:pd.DataFrame):
        return [ self.sequence[record['start']-1:record['end']] for record in orfs.to_dict('records')]
